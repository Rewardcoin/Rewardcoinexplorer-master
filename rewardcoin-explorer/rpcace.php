...
define( 'RPCPORT', 10086 );
define( 'RPCUSER', 'abc' );
define( 'RPCPASS', '123' );

define( 'COINNAME', 'Rewardcoin' );
...
define( 'COINHOME', 'https://bitcointalk.org/index.php?topic=1175701.0' );
...
class RPCAce
{
    ...
    private static function base()
    {
        ...
        if( !($hashRate = @$rpc->getmininginfo()['netmhashps']) && !($hashRate = @$rpc->getmininginfo()['networkhashps']) )
            $hashRate = $rpc->getnetworkhashps();
        $output['hashrate_mhps'] = sprintf( '%.0f', $hashRate );
...
$diffNom = 'Difficulty';
$diff = sprintf( '%.8f', $ace['current_difficulty_pow'] );
if( COINPOS )
{
    $diffNom .= ' &middot; PoS';
...
if( isset($ace['blocks']) )
{
    ...
    foreach( $ace['blocks'] as $block )
        echo "<tr class=\"illu\"><td>{$block['height']}</td><td><a href=\"?{$block['hash']}\">" . substr( $block['hash'], 0, 16 ) . ' ...</a></td><td>' . sprintf( '%.8f', $block['difficulty'] ) . "</td><td><a title=\"{$block['time']}\">{$block['date']}</a></td><td>{$block['tx_count']} &middot; " . sprintf( '%.2f', $block['total_out'] ) . '</td></tr>';
